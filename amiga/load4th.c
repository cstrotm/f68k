...
/* DEBUG option removed due to problems with call from Workbench */
...
#include <exec/exec.h>  /* only needed for switching 
 								   to SuperVisor state */
...
typedef void FUNC(struct forthparas*);
	...
	if (options)	
	{
		...
		if (!dumpfile)	Write(handle,"Dumpfile will nicht\n");	
	}
